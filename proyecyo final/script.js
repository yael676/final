function calcularCosto(event) {
    ...
    let cuotaMensual = 125; // Base para Licenciatura
    let matriculaCiclo = 200; // Base para Licenciatura
    let pagoLaboratorios = 0;

    // Ajustes según la carrera seleccionada
    switch (carrera) {
        case "medicina":
            cuotaMensual += cuotaMensual * 0.75; // Aumenta 75%
            matriculaCiclo += matriculaCiclo * 1.25; // Aumenta 125%
            pagoLaboratorios = 30; // Costo de laboratorios para Medicina por ciclo
            break;
        case "ingenieria":
            cuotaMensual += cuotaMensual * 0.50; // Aumenta 50%
            matriculaCiclo += matriculaCiclo * 0.50; // Aumenta 50%
            pagoLaboratorios = 7; // Costo de laboratorios para Ingeniería por ciclo
            break;
        // Licenciatura no requiere ajustes, permanece en los valores base
    }
